...
class JobMatchingService
{
    ...
    protected function skillsBasedMatching(ParsedResume $parsedResume, Collection $workExperience, int $limit): Collection
    {
        ...
        // Build query for skills-based matching
        $jobs = Lead::select('*')
            ->selectRaw('
                (CASE 
            ...
            ->get()
            ->map(function ($job) use ($userSkills, $jobTitles, $parsedResume) {
            ...
            ->filter(function ($job) {
                return $job->relevance_score >= 15; // Lower threshold due to penalty system
            })
    ...
    protected function experienceBasedMatching(Collection $workExperience, int $limit): Collection
    {
        ...
        $jobs = Lead::select('*')
            ...
            ->where('is_active', true)
            ->get()
            ->map(function ($job) use ($keywords, $jobTitles) {
            ...
            ->filter(function ($job) {
                return $job->relevance_score >= 20; // Lower threshold for experience-based
            })
            ->sortByDesc('relevance_score')
            ->take($limit);
